...
describe('empleadoService', () => {
    ...
    describe('crearEmpleado', () => {
        ...
        it('debe crear un empleado exitosamente', async () => {
            ...
            empleadoRepository.create.mockResolvedValue({ /* datos del empleado creado */ });

            const result = await empleadoService.crearEmpleado(/* parámetros del empleado */);

            expect(result).toEqual({ /* datos esperados del empleado */ });
        ...
        it('debe lanzar un error si el nick ya está en uso', async () => {
            empleadoRepository.findByNick.mockResolvedValue({ /* empleado existente con el mismo nick */ });

            await expect(empleadoService.crearEmpleado(/* parámetros del empleado */)).rejects.toThrow('El nick ya está siendo utilizado por otro empleado');
        });
    ...
    describe('updateEmpleado', () => {
        ...
        it('debe actualizar un empleado exitosamente', async () => {
            empleadoRepository.findByNick.mockResolvedValue(null);
            empleadoRepository.update.mockResolvedValue({ /* datos del empleado actualizado */ });

            const result = await empleadoService.updateEmpleado(/* parámetros del empleado */);
    ...
    describe('authenticate', () => {
        ...
        it('debe autenticar un empleado exitosamente', async () => {
            empleadoRepository.findByNick.mockResolvedValue({ /* datos del empleado */ });
            bcrypt.compare.mockResolvedValue(true);

            const result = await empleadoService.authenticate(/* nick, password */);
