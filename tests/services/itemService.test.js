...
describe('createItemsForOrder', () => {
    ...
    it('should create items for order successfully', async () => {
        ...
        itemMenuRepository.findItemInventarios.mockResolvedValue([mockItemInventario]);
        itemInventarioRepository.getStock.mockResolvedValue(10); // Supongamos que hay suficiente stock
        itemRepository.createItem.mockResolvedValue({ id: 1, ...mockData.items[0], precio: mockItemMenu.precio });

        const result = await itemService.createItemsForOrder(mockData, mockOrder, null);
...
describe('manejarStockItemsOnCancelar', () => {
    ...
    it('should handle stock items on cancel correctly', async () => {
        ...
        itemMenuRepository.getItemMenuById.mockResolvedValue({ id: 1, nombre: 'Item Menu' });
        itemMenuRepository.findItemInventarios.mockResolvedValue([{ id: 1, porUnidad: true }]);
