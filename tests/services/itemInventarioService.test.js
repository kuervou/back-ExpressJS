...
describe('itemInventarioService', () => {
    ...
    describe('crearItemInventario', () => {
        it('debe crear un item de inventario exitosamente', async () => {
            ...
            categoriaRepository.getCategoriaById.mockResolvedValue({ /* datos de la categoría */ });
            itemInventarioRepository.create.mockResolvedValue({ /* datos del item creado */ });

            const result = await itemInventarioService.crearItemInventario(/* parámetros del item */);

            expect(result).toEqual({ /* datos esperados del item */ });
        ...
        it('debe lanzar un error si el nombre ya está en uso', async () => {
            itemInventarioRepository.findByNombre.mockResolvedValue({ /* item existente con el mismo nombre */ });

            await expect(itemInventarioService.crearItemInventario(/* parámetros del item */)).rejects.toThrow('Ya existe un item con ese nombre');
        });
        ...
        it('debe lanzar un error si la categoría no existe', async () => {
            ...
            await expect(itemInventarioService.crearItemInventario(/* parámetros del item */)).rejects.toThrow('Categoría no encontrada');
        });
    ...
    describe('updateItemInventario', () => {
        ...
        it('debe actualizar un item de inventario exitosamente', async () => {
            itemInventarioRepository.findByNombre.mockResolvedValue(null);
            categoriaRepository.getCategoriaById.mockResolvedValue({ /* datos de la categoría */ });
            itemInventarioRepository.update.mockResolvedValue({ /* datos del item actualizado */ });

            const result = await itemInventarioService.updateItemInventario(/* parámetros del item */);
    ...
    describe('updateStock', () => {
        ...
        it('debe actualizar el stock de un item exitosamente', async () => {
            itemInventarioRepository.getItemInventarioById.mockResolvedValue({ /* datos actuales del item */ });
            itemInventarioRepository.updateStock.mockResolvedValue({ /* datos del item con stock actualizado */ });

            const result = await itemInventarioService.updateStock(/* id, cantidad a sumar o restar */);

            expect(result).toEqual({ /* datos esperados del item con stock actualizado */ });
            // Agrega más expectativas según sea necesario
        });
