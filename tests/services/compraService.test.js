...
describe('compraService', () => {
    ...
    describe('crearCompra', () => {
        it('debe crear una compra exitosamente', async () => {
            ...
            empleadoRepository.getEmpleadoById.mockResolvedValue({ /* datos del empleado */ });
            itemInventarioRepository.getItemInventarioById.mockResolvedValue({ /* datos del itemInventario */ });
            compraRepository.create.mockResolvedValue({ /* datos de la compra creada */ });
            db.sequelize.transaction.mockResolvedValue({ commit: jest.fn(), rollback: jest.fn() });

            const data = { /* datos de la compra */ };
            const result = await compraService.crearCompra(data);

            expect(result).toEqual({ /* datos esperados de la compra */ });
        ...
        it('debe lanzar un error si el empleado no existe', async () => {
            ...
            const data = { /* datos de la compra */ };
            await expect(compraService.crearCompra(data)).rejects.toThrow('Empleado no encontrado');
        });
        ...
        it('debe lanzar un error si el itemInventario no existe', async () => {
            // Mockea las dependencias
            empleadoRepository.getEmpleadoById.mockResolvedValue({ /* datos del empleado */ });
            itemInventarioRepository.getItemInventarioById.mockResolvedValue(null);

            const data = { /* datos de la compra */ };
            await expect(compraService.crearCompra(data)).rejects.toThrow('ItemInventario no encontrado');
        });
    ...
    describe('getCompras', () => {
        it('debe obtener todas las compras', async () => {
            compraRepository.findAll.mockResolvedValue([/* Array de compras */]);
            const options = { /* opciones de filtro */ };
            const result = await compraService.getCompras(options);

            expect(result).toEqual([/* Array esperado de compras */]);
            // Agrega más expectativas según sea necesario
        });
    });
    ...
    describe('getCompraById', () => {
        it('debe obtener una compra por ID', async () => {
            compraRepository.getCompraById.mockResolvedValue({ /* datos de una compra */ });
    ...
    describe('deleteCompra', () => {
        it('debe eliminar una compra', async () => {
            compraRepository.getCompraById.mockResolvedValue({ /* datos de la compra */ });
            itemInventarioRepository.getItemInventarioById.mockResolvedValue({ /* datos del itemInventario */ });
            compraRepository.deleteCompra.mockResolvedValue({ /* confirmación de eliminación */ });
            db.sequelize.transaction.mockResolvedValue({ commit: jest.fn(), rollback: jest.fn() });

            const id = 1; // Ejemplo de ID
            const result = await compraService.deleteCompra(id);

            expect(result).toEqual({ /* datos esperados después de la eliminación */ });
            // Agrega más expectativas según sea necesario
        });
