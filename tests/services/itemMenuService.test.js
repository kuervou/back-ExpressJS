...
describe('itemMenuService', () => {
    ...
    describe('crearItemMenu', () => {
        it('debe crear un item de menú exitosamente', async () => {
            ...
            grupoRepository.getGrupoById.mockResolvedValue({ /* datos del grupo */ });
            itemMenuRepository.create.mockResolvedValue({ /* datos del itemMenu creado */ });
            itemInventarioRepository.getItemInventarioById.mockResolvedValue({ /* datos del itemInventario */ });
            itemMenuRepository.addItemInventario.mockResolvedValue({ /* confirmación de asociación */ });

            const data = { /* datos para crear el itemMenu */ };
            const result = await itemMenuService.crearItemMenu(data);

            expect(result).toEqual({ /* datos esperados del itemMenu creado */ });
        ...
        it('debe lanzar un error si el grupo no existe', async () => {
            ...
            const data = { /* datos para crear el itemMenu */ };
            await expect(itemMenuService.crearItemMenu(data)).rejects.toThrow('Grupo no encontrado');
        });
    ...
    describe('updateItemMenu', () => {
        ...
        it('debe actualizar un item de menú exitosamente', async () => {
            itemMenuRepository.findByNombre.mockResolvedValue(null);
            grupoRepository.getGrupoById.mockResolvedValue({ /* datos del grupo */ });
            itemMenuRepository.update.mockResolvedValue({ /* datos del itemMenu actualizado */ });

            const id = 1;
            const data = { /* datos para actualizar el itemMenu */ };
            const result = await itemMenuService.updateItemMenu(id, data);

            expect(result).toEqual({ /* datos esperados del itemMenu actualizado */ });
            // Agrega más expectativas según sea necesario
        });
