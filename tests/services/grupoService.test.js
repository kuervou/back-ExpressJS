...
describe('grupoService', () => {
    describe('crearGrupo', () => {
        ...
        it('should successfully create a new group', async () => {
            ...
            const result = await grupoService.crearGrupo(mockNombre, mockEsBebida);
    
            expect(grupoRepository.findByNombre).toHaveBeenCalledWith(mockNombre.toLowerCase());
            expect(grupoRepository.create).toHaveBeenCalledWith(mockNombre, mockEsBebida);
        ...
        it('should throw an error when group name already exists', async () => {
            ...
            await expect(grupoService.crearGrupo(mockNombre, mockEsBebida))
                .rejects
                .toThrow('Ya existe un grupo con ese nombre');
    ...
    describe('updateGrupo', () => {
        ...
        it('should throw an error when group name already exists in another group', async () => {
            grupoRepository.findByNombre.mockResolvedValue({ id: 2, nombre: 'Otro grupo' });
    
            await expect(grupoService.updateGrupo(1, 'Otro grupo', true))
                .rejects
                .toThrow('Ya existe un grupo con ese nombre');
        });
    });
