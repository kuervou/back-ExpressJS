...
describe('movimientoService', () => {
    ...
    describe('crearMovimiento', () => {
        it('should create a movimiento successfully', async () => {
            const movimientoData = { /* ... datos del movimiento ... */ };
            const mockMovimiento = { /* ... datos del movimiento creado ... */ };
            movimientoRepository.create.mockResolvedValue(mockMovimiento);
            cajaRepository.getCajaById.mockResolvedValue({ /* ... datos de la caja ... */ });
            empleadoRepository.getEmpleadoById.mockResolvedValue({ /* ... datos del empleado ... */ });

            const result = await movimientoService.crearMovimiento(movimientoData);
    ...
    describe('getMovimientos', () => {
        it('should retrieve movimientos successfully', async () => {
            const options = { /* ... opciones de filtrado ... */ };
            const mockMovimientos = [/* ... lista de movimientos ... */];
    ...
    describe('getMovimientoById', () => {
        it('should retrieve a movimiento by id successfully', async () => {
            const id = 1;
            const mockMovimiento = { /* ... datos del movimiento ... */ };
    ...
    describe('getMovimientosByCajaId', () => {
        it('should retrieve movimientos by caja id successfully', async () => {
            ...
            const options = { /* ... opciones de filtrado ... */ };
            const mockMovimientos = [/* ... lista de movimientos ... */];
            movimientoRepository.getMovimientosByCajaId.mockResolvedValue(mockMovimientos);

            const result = await movimientoService.getMovimientosByCajaId(id, options);
            expect(result).toEqual(mockMovimientos);
        });
    });
