...
describe('updateCategoria', () => {
    ...
    it('should throw an error when categoria name already exists in another categoria', async () => {
        ...
        categoriaRepository.findByNombre.mockResolvedValue({ id: 2, nombre: mockNombre });

        await expect(categoriaService.updateCategoria(1, mockNombre))
            .rejects
            .toThrow('Ya existe una categoría con ese nombre');
    });
});
...
describe('crearCategoria', () => {
    it('should successfully create a new categoria', async () => {
        const mockNombre = 'Electrónica';
        categoriaRepository.create.mockResolvedValue({ id: 1, nombre: mockNombre });
        categoriaRepository.findByNombre.mockResolvedValue(null);
    ...
    it('should throw an error when categoria name already exists', async () => {
        ...
        await expect(categoriaService.crearCategoria(mockNombre))
            .rejects
            .toThrow('Ya existe una categoría con ese nombre');
    });
});
