...
describe('ordenController', () => {
    ...
    describe('POST /api/ordenes', () => {
        ...
        it('should create a new order successfully', async () => {
            const ordenData = {
                ...
                ocupacion: 4,
                items: [
                    { itemMenuId: 1, cantidad: 2, precio: 10.00 },
                    { itemMenuId: 2, cantidad: 1, precio: 15.00 }
                ],
    ...
    describe('POST /api/ordenes/pagarTodo', () => {
        beforeEach(() => {
            ...
            cajaService.getCajaById.mockResolvedValue({ id: 1, monto: 1000, estado: 'Abierta' });
            empleadoService.getEmpleadoById.mockResolvedValue({ id: 1, nombre: 'Juan', apellido: 'Perez', rol: 'Mozo' });
            ordenService.getOrdenesByIds.mockResolvedValue([
                { id: 1, fecha: '2021-09-01', hora: '12:00:00', estado: 'En cocina', ocupacion: 4, items: [], paga: false },
                { id: 2, fecha: '2021-09-01', hora: '12:00:00', estado: 'En cocina', ocupacion: 4, items: [], paga: false }
            ]);
        });
        ...
        it('should process the payment for all orders successfully', async () => {
            ...
            empleadoService.getEmpleadoById.mockResolvedValue({ id: empleadoId });
            ...
            ordenService.pagarTodo.mockResolvedValue({ /* tu respuesta esperada del servicio */ });
            ...
            expect(ordenService.pagarTodo).toHaveBeenCalledWith(expect.objectContaining({
                ordenesNoPagadas: expect.arrayContaining([
                    expect.objectContaining({ id: expect.any(Number), paga: false })
                ]),
                cajaId,
                empleadoId,
                metodoPago,
        ...
        it('should return an error if ordenes does not exist', async () => {
            ...
            const res = await request(app)
                ...
                .send({ ordenes: ordenesIds, cajaId, empleadoId, metodoPago });
            expect(res.statusCode).toEqual(404);
            expect(res.body).toHaveProperty('message', 'Alguna de las ordenes no existe');
    ...
    describe('GET /api/ordenes', () => {
        ...
        it('should retrieve orders with specific filters', async () => {
            ...
            expect(ordenService.getOrdenes).toHaveBeenCalledWith(expect.objectContaining(filterData));
        });
    ...
    describe('GET /api/ordenes/caja', () => {
        ...
        it('should retrieve orders associated with a specific caja filtered by mesaId', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedOrders);
            expect(ordenService.getOrdenesCaja).toHaveBeenCalledWith({ mesaId: mesaId });
        });
    ...
    describe('GET /api/ordenes/mozo', () => {
        ...
        it('should retrieve orders associated with a specific mozo filtered by mesaId', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedOrders);
            expect(ordenService.getOrdenesMozo).toHaveBeenCalledWith({ mesaId: mesaId });
        });
    ...
    describe('GET /api/ordenes/historial', () => {
        ...
        it('should retrieve orders associated with a specific mozo filtered by mesaId', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedOrders);
            expect(ordenService.getOrdenesHistorial).toHaveBeenCalledWith({ mesaId: mesaId });
        });
    ...
    describe('GET /api/ordenes/estadisticas/ventas', () => {
        ...
        it('should retrieve sales statistics for a specific day', async () => {
            ...
            expect(ordenService.getEstadisticasVentas).toHaveBeenCalledWith({ dia: fecha });
        });
        ...
        it('should retrieve sales statistics for a specific month', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedStats);
            expect(ordenService.getEstadisticasVentas).toHaveBeenCalledWith({ mes });
        });
        ...
        it('should retrieve sales statistics for a specific year', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedStats);
            expect(ordenService.getEstadisticasVentas).toHaveBeenCalledWith({ anio });
        });
    ...
    describe('GET /api/ordenes/estadisticas/cantOrdenesProcesadas', () => {
        ...
        it('should retrieve the count of processed orders for a specific day', async () => {
            ...
            ordenService.getCantOrdenesProcesadas.mockResolvedValue(expectedCount);
    
            const res = await request(app)
                .get(`/api/ordenes/estadisticas/cantOrdenesProcesadas?dia=${fecha}`)
                .set('Authorization', authToken);
    
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedCount);
            expect(ordenService.getCantOrdenesProcesadas).toHaveBeenCalledWith({ dia: fecha });
        });
        ...
        it('should retrieve the count of processed orders for a specific month', async () => {
            ...
            const res = await request(app)
                .get(`/api/ordenes/estadisticas/cantOrdenesProcesadas?mes=${mes}`)
                .set('Authorization', authToken);
    
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedCount);
            expect(ordenService.getCantOrdenesProcesadas).toHaveBeenCalledWith({ mes });
        });
        ...
        it('should retrieve the count of processed orders for a specific year', async () => {
            ...
            const res = await request(app)
                .get(`/api/ordenes/estadisticas/cantOrdenesProcesadas?anio=${anio}`)
                .set('Authorization', authToken);
    
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedCount);
            expect(ordenService.getCantOrdenesProcesadas).toHaveBeenCalledWith({ anio });
        });
    ...
    describe('GET /api/ordenes/estadisticas/consumoPorClienteId/:id', () => {
        ...
        it('should retrieve the consumption for a specific client for a specific day', async () => {
            ...
            ordenService.getConsumoPorClienteId.mockResolvedValue(expectedConsumption);
    
            const res = await request(app)
                .get(`/api/ordenes/estadisticas/consumoPorClienteId/1?dia=2023-04-01`)
                .set('Authorization', authToken);
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedConsumption);
            expect(ordenService.getConsumoPorClienteId).toHaveBeenCalledWith(clienteId, { dia: fecha });
        });
        ...
        it('should retrieve the consumption for a specific client for a specific month', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedConsumption);
            expect(ordenService.getConsumoPorClienteId).toHaveBeenCalledWith(clienteId, { mes });
        });
        ...
        it('should retrieve the consumption for a specific client for a specific year', async () => {
            ...
            const res = await request(app)
                .get(`/api/ordenes/estadisticas/consumoPorClienteId/1?anio=2023`)
                .set('Authorization', authToken);
    
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedConsumption);
            expect(ordenService.getConsumoPorClienteId).toHaveBeenCalledWith(clienteId, { anio });
        });
    ...
    describe('GET /ordenes/estadisticas/consumoClientes', () => {
        ...
        it('should retrieve the consumption statistics for a specific day', async () => {
            ...
            ordenService.getConsumoClientes.mockResolvedValue(expectedStatistics);
            ...
            expect(ordenService.getConsumoClientes).toHaveBeenCalledWith({ dia: fecha });
        });
        ...
        it('should retrieve the consumption statistics for a specific month', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedStatistics);
            expect(ordenService.getConsumoClientes).toHaveBeenCalledWith({ mes });
        });
        ...
        it('should retrieve the consumption statistics for a specific year', async () => {
            ...
            expect(res.statusCode).toEqual(200);
            expect(res.body).toEqual(expectedStatistics);
            expect(ordenService.getConsumoClientes).toHaveBeenCalledWith({ anio });
        });
    ...
    describe('GET /api/ordenes/estadisticas/top5Clientes', () => {
        ...
        it('should retrieve the top 5 clients for a specific day', async () => {
            ...
            expect(ordenService.getTop5Clientes).toHaveBeenCalledWith({ dia: fecha });
        });
    ...
    describe('GET /api/ordenes/estadisticas/top5ItemsMenu', () => {
        ...
        it('should retrieve the top 5 menu items for a specific day', async () => {
            ...
            expect(ordenService.getTop5ItemsMenu).toHaveBeenCalledWith({ dia: fecha });
        });
    ...
    describe('PUT /api/ordenes/:id', () => {
        ...
        it('should update an order successfully', async () => {
            ...
            expect(ordenService.updateOrden).toHaveBeenCalledWith(ordenId, updateData);
        });
    ...
    describe('PUT /api/ordenes/:id/mesas', () => {
        ...
        it('should add mesas to an order successfully', async () => {
            ...
            ordenService.addMesas.mockResolvedValue({ message: 'Mesas agregadas' });
            ...
            expect(ordenService.addMesas).toHaveBeenCalledWith(ordenId, mesasToAdd);
        });
    
        it('should return error if order not found', async () => {
            const mesasToAdd = [2, 3];
    
            ordenService.addMesas.mockRejectedValue(new HttpError(HttpCode.NOT_FOUND, 'Orden no encontrada'));
    ...
    describe('DELETE /api/ordenes/:id/mesas', () => {
        ...
        it('should remove mesas from an order successfully', async () => {
            ...
            ordenService.removeMesas.mockResolvedValue({ message: 'Mesas removidas' });
            ...
            expect(ordenService.removeMesas).toHaveBeenCalledWith(ordenId, mesasToRemove);
        });
    
        it('should return error if order not found', async () => {
            const mesasToRemove = [2, 3];
    
            ordenService.removeMesas.mockRejectedValue(new HttpError(HttpCode.NOT_FOUND, 'Orden no encontrada'));
    ...
    describe('POST /api/ordenes/:id/items', () => {
        ...
        it('should add items to an order successfully', async () => {
            ...
            ordenService.addItems.mockResolvedValue({ message: 'Items agregados' });
            ...
            expect(ordenService.addItems).toHaveBeenCalledWith(ordenId, { items: itemsToAdd });
        });
        ...
        it('should return error if order not found', async () => {
            ...
            ordenService.addItems.mockRejectedValue(new HttpError(HttpCode.NOT_FOUND, 'Orden no encontrada'));
    ...
    describe('DELETE /api/ordenes/:id/items', () => {
        ...
        it('should remove items from an order successfully', async () => {
            ...
            ordenService.removeItems.mockResolvedValue({ message: 'Items removidos' });
            ...
            expect(ordenService.removeItems).toHaveBeenCalledWith(ordenId, itemsToRemove);
        });
    
        it('should return error if order not found or invalid items provided', async () => {
            const itemsToRemove = [1, 2];
    
            ordenService.removeItems.mockRejectedValue(new HttpError(HttpCode.NOT_FOUND, 'Orden no encontrada o items inválidos'));
            ...
            expect(res.statusCode).toBeGreaterThanOrEqual(400);
            expect(res.body).toHaveProperty('message', 'Orden no encontrada o items inválidos');
        });
    ...
    describe('DELETE /api/ordenes/:id', () => {
        ...
        it('should return error if order not found or cannot be deleted', async () => {
            ...
            ordenService.deleteOrden.mockRejectedValue(new HttpError(HttpCode.NOT_FOUND, 'Orden no encontrada o no se puede eliminar'));
            ...
            expect(res.statusCode).toBeGreaterThanOrEqual(400);
            expect(res.body).toHaveProperty('message', 'Orden no encontrada o no se puede eliminar');
        });
